package lunatech.before.resources;
...
//                .map(maybeUser -> tagsFilter.map(filter -> {
//                    var tagsList = Arrays.stream(filter.split(",")).toList();
//                    System.out.println(tagsList);
//                    return maybeUser.map(user -> user.todos.stream()
//                            .filter(todo -> new HashSet<>(todo.tags).containsAll(tagsList)).toList());
//                }).orElse(maybeUser.map(user -> user.todos)));
//        return todos
//                .map(Response::ok)
...
//        var userTarget =  userName.orElse(authService.userName());
//
//        var eitherTodo = userService.queryUser(userTarget)
//                .map(maybeUser -> maybeUser.map(
//                        user -> user.todos.stream().filter(todo -> todo.todoId.equals(id))));
//
//        return eitherTodo
//                .map(Response::ok)
//                .getOrElseGet(error -> Response.status(Response.Status.BAD_REQUEST).entity(error))
